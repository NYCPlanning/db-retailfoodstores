...
OR upper(dba_name) LIKE '%SUPERMARKET%'
OR upper(dba_name) LIKE '%GROCERY%');

-- Categorize the supermarkets with store names or entity names
UPDATE dcp_retailfoodstores
SET dcp_type = 'Supermarket'
WHERE upper(dba_name) LIKE '%TRADER%JOE%'
...
WHERE square_footage <= 4000
AND dcp_type IS NULL
AND (upper(dba_name) LIKE '%CONVENIENT%'
OR upper(dba_name) LIKE '%CONVENIENCE%'
OR upper(dba_name) LIKE '%LOTTO%'
OR upper(dba_name) LIKE '%TOBACCO%'
OR upper(dba_name) LIKE '%CANDY%'
OR upper(dba_name) LIKE '%SNACK%'
OR upper(dba_name) LIKE '%GOURMET%'
...
OR upper(dba_name) LIKE '%CHOCOLATE%'
